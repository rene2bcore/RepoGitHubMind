...
> **Estado a 2026-09-14:** Entrega 1, documentación técnica. Todavía no hay código: la estructura, los comandos y el contrato descritos aquí son los decididos, y se materializan con la primera historia en la Entrega 2. Está registrado en [H-01](docs/hallazgos.md).
...
Monolito modular en un monorepo pnpm: `apps/web` (UI, Auth.js, Route Handlers y Server Actions), `apps/worker` (cola de trabajos en PostgreSQL: fetch de GitHub, análisis de IA, embeddings), y `packages/` con `db`, `github`, `ai`, `search`, `shared` y `config`. Sin microservicios, sin Redis, sin Elasticsearch, sin vector DB externa.
...
|---|---|---|---|
| [H1 · Cuentas y sesión](docs/backlog/RGM-2-cuentas-y-sesion.md) | RGM-2 | must-have | Por hacer, con tickets RGM-9 (datos), RGM-10 (backend) y RGM-11 (frontend) |
...
|---|---|---|
| `DATABASE_URL` | PostgreSQL de desarrollo | `postgres://rgm:rgm@localhost:5432/repogithubmind` |
| `AUTH_SECRET` | Secreto de Auth.js. Se genera con `openssl rand -base64 32` | vacío |
