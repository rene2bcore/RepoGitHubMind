...
Tu objetivo es **refutar el cambio, no aprobarlo**. Contrasta contra `docs/specs/`, `docs/api/openapi.json` y `CLAUDE.md`, no contra tu gusto.
...
- **Lo que ya vigila una comprobación**: lint, formato, tipos, las pruebas, la comparación del contrato, `scripts/verificar-docs.mjs`, los scripts `probar-*` y el catálogo de `scripts/mutaciones.mjs`. Si un script ya lo muerde, un modelo mirándolo es gasto.
- **Ficheros generados** y `node_modules`. **`docs/api/openapi.json` no está en esta lista** aunque se genere: es el contrato, y lo que afirma sobre autorización y respuestas sí se revisa.
- **Estilo, nombres, orden, «esto podría extraerse a una función»**, y rendimiento sin un número que lo respalde.
- **Huecos ya declarados** en `docs/traceability.md`. Que un hueco esté declarado no cubre lo que haya a su lado.
