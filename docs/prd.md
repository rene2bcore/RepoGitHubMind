...
- **RF-9** _(S1)_ · De un texto pegado o un `.txt` se extraen las URLs de GitHub, se deduplican y se muestra un preview con nuevos, ya guardados e inválidos antes de importar.
...
- **RF-17** _(S2)_ · El detalle muestra repositorios similares de mi biblioteca y del corpus global sin revelar quién los guardó.
...
- **Stack** (maestro §9): Next.js App Router, React, TypeScript strict, Tailwind, shadcn/ui, PostgreSQL 16 + pgvector, Drizzle, sesión propia en R1 y Auth.js con OAuth en R2 ([ADR-0013](adr/0013-sesion-propia-en-vez-de-authjs.md)), Zod, Vitest, Playwright, pnpm workspaces, Docker Compose. Sin microservicios, sin Redis, sin Elasticsearch, sin vector DB externa, sin IA local.
...
| PA-2 | **Cerrado el 2026-09-14.** OpenRouter con `google/gemini-2.5-flash-lite` por defecto (0,10 USD/M entrada, 0,40 USD/M salida, `json_schema`), elegible con `AI_MODEL_ANALYSIS`. Medido con una llamada real sobre `pgvector/pgvector`: 4 401 + 672 tokens, 0,0007089 USD. Sin la medición de 20 repositorios ([`ai-architecture.md`](ai-architecture.md)) | H4                          | El autor                             |
| PA-3 | **Cerrado el 2026-09-14.** `openai/text-embedding-3-small` por OpenRouter, dimensión 1536 (verificado). La tabla `repository_embeddings` se crea con H5                                                                                                                                                                                                  | H5                          | El autor                             |
