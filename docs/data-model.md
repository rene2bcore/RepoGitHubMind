...
> Derivado del [prompt maestro](prompts/00-prompt-maestro.md) §3, §13, §14, §38 y §69. Describe el esquema de `packages/db/src/schema.ts` y sus migraciones `0000` a `0005` (H1, H2, H4 y H5), que es lo que manda.
...
erDiagram
    ...
    repositories {
        ...
        tsvector search_vector "documento lexico: nombre descripcion topics categorias resumen proposito casos de uso tags"
        timestamptz created_at
        timestamptz updated_at
    }
    repository_analyses {
    ...
    ai_usage {
        uuid id PK
        uuid repository_id FK "set null al borrar el repositorio; null en una consulta de busqueda"
        text provider
        text model
...
`sessions` guarda las sesiones propias de R1 ([ADR-0013](adr/0013-sesion-propia-en-vez-de-authjs.md)): cerrar sesión borra la fila. `accounts` y `verification_tokens` (no dibujada) tienen la forma que el adaptador de Drizzle para Auth.js exige y se crean ya para no migrar dos veces cuando entren GitHub OAuth y Google en R2; en R1 están vacías. Columnas reales en `packages/db/src/schema.ts` y en `packages/db/migrations/`. Con H2 existen `repositories`, `repository_analyses`, `user_repositories` y `background_jobs` (migración `0001`); con H4, `categories`, `repository_categories`, `tags`, `repository_tags` y `ai_usage` (migración `0002`), con el catálogo sembrado por `packages/db/src/taxonomy.ts` en todos los entornos; con H5, la extensión `vector`, `repository_embeddings` y `repositories.search_vector` con su índice GIN (migraciones `0003` a `0005`). La `0003` y la `0005` son personalizadas (`drizzle-kit generate --custom`): Drizzle no genera `CREATE EXTENSION` ni datos. La `0005` rellena `search_vector` de los repositorios que ya existían, con una copia congelada de `searchDocument()`, y encola `GENERATE_EMBEDDING` para los análisis completados. `search_vector` no es una columna generada porque mezcla el resumen, las categorías y los tags, que viven en otras tablas: lo escribe `refreshSearchVector` (`packages/search/src/document.ts`) al dar de alta el repositorio y al completar su análisis, con la configuración de texto `simple`, porque el documento mezcla inglés y castellano. Si `pg-boss` resulta la cola elegida ([ADR-0010](adr/0010-cola-de-trabajos-en-postgresql.md)), `background_jobs` la sustituye su propio esquema `pgboss` y aquí queda solo la vista de estados que lee la interfaz.
...
| Toda query sobre `user_repositories` filtra por el `user_id` de la sesión                                                                                                                                                                                                               | `apps/web/src/modules/repositories/service.ts`       | prueba de integración con dos cuentas                                                                  | `specs/library`: «Lo mío no lo ve nadie»              |
| Los documentos de búsqueda (`search_vector`, `repository_embeddings`) salen solo del repositorio global y su análisis; en la búsqueda, lo personal es siempre de la cuenta de la sesión, también en `global`                                                                            | `packages/search/src/document.ts`, `loadSearchItems` | `search.test.ts` con dos cuentas; mutaciones `busqueda-global-privada` y `busqueda-biblioteca-privada` | `specs/search`: «Ámbitos y privacidad»                |
