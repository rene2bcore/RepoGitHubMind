...
| Variables     | Las de `.env.example` con valores reales en un `.env` del servidor, nunca en el repositorio. `AUTH_URL` con el dominio real, `AUTH_SECRET` propio, `AI_API_KEY`, `GITHUB_TOKEN` opcional, `DEBUG_HTTP_ERRORS=false`, `LOG_LEVEL=info`, `TRUST_PROXY=true` porque Cloudflare escribe `cf-connecting-ip` ([H-07](hallazgos.md)) |
