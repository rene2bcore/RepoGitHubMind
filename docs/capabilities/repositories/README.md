...
| `GET /api/v1/repositories`  | `libraryQuerySchema`: `status`, `favorite`, `language`, `license`, `minStars`, `sort`, `page`, `pageSize` | `GET`   | `200 { data: UserRepository[], meta: { total, page, pageSize } }`; `422` valor fuera del dominio | sí     |
...
| Worker            | `apps/worker/src/jobs.ts`, `apps/worker/src/index.ts` | Consume la cola. En H2 `ANALYZE_REPOSITORY` solo fija `DISABLED` o `FAILED` con motivo; el análisis real llega con H4                              |
...
| Guardar responde con el análisis `PENDING` y encola; con la IA apagada, `DISABLED` | Guardar responde antes que la IA; La IA falla | `upsertRepository`, `apps/worker/src/jobs.ts`                   | `repositories.test.ts` · PENDING/DISABLED; `cola.test.ts`                                          |
