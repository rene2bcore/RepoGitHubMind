...
/** El error de un parámetro de query repetido; lo usan la API y las páginas que leen la URL. */
export function repeatedParam(field: string) {
  return { field, rule: 'repeated', message: 'Este parámetro solo puede venir una vez' }
}
...
export function parseQuery<S extends z.ZodType>(req: Request, schema: S): z.output<S> {
  ...
  if (repeated.length) throw new ValidationError(repeated.map(repeatedParam))
